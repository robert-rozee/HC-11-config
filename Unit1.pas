...
type
  TForm1 = class(TForm)
    ...
    GroupBox4: TGroupBox;
      UpDown2: TUpDown;                     // address up/down
      Label4: TLabel;                       // address display
    GroupBox2: TGroupBox;
    ...
    GroupBox3: TGroupBox;
      ...
      RadioButton6: TRadioButton;           // 8 data bits
...
begin
  ...
  if (length(s)>=3) and (s[3]='+') then s[3]:='-';  // HC-11 only, "+" -> "-"
  at2ok:=s
end;
...
begin
  ...
  begin
    ...
    ChannelUpdate;
    UPDown2.Position:=0;                         // address 0
    AddressUpdate;
    ComboBox1.ItemIndex:=3;                      // 9600 baud
    ComboBox2.ItemIndex:=0;                      // FU1
    ComboBox3.ItemIndex:=7;                      // 10mw/10dBm
...
// FormActivate as the comm port hasn't been selected yet.
   ...
   CheckBox1.Checked:=false;                    // LPD433 UNLOCKED
   InitControls                      // (we don't know frequencies for HC-11)
end;                                 // *************************************
...
// ************************************************************************
// HC-11: only limit seems to be that in mode FU2, max baud rate is 4800bps
// ************************************************************************
...
// send factory reset command "AT+RESET"
  s:='AT+RESET'+EOL;
  e:=at2ok(s);
...
(*
  ...
  2.4 write "AT+V"
  2.5 read  "HC-11_V1.3"
  (parity, #stop bits are NOT returned)
 *)
...
// get read all configurations response
  ...
  while (got<>0) and (b[got]<#32) do dec(got);
...
(*
  ...
  3.6  read  "OK+Cxxx"
  3.7  response bad
  3.8  write "AT+Axxx"         // set address (000 .. 255)
  3.9  read  "OK+Axxx"
  3.10 response bad
  ...
  3.15 read  "OK+Px"
  3.16 response bad
  3.17 write "AT+Ups"          // set serial format (parity, #stop bits)
  3.18 read  "OK+Ups"
  3.19 response bad
 *)
...
// send set address command "AT+Axxx"
  s:='AT+A'+Label4.Caption+EOL;
  e:=at2ok(s);

  try
    if not WriteFile(CommFile, s[1], length(s), put, nil) then  // write command
    begin
      showmessage('Serial I/O error: WriteFile failed (3.8)');
      halt
    end
  except
    showmessage('Serial I/O error: WriteFile exception (3.8)');
    halt
  end;
...
// get set channel/frequency response
  try
    if not ReadFile(CommFile, b[1], sizeof(b), got, nil) then   // read response
    begin
      showmessage('Serial I/O error: ReadFile failed (3.9)');
      halt
    end
  except
    showmessage('Serial I/O error: ReadFile exception (3.9)');
    halt
  end;

  r:=copy(b,1,got);
  if strip(r)<>strip(e) then
     showmessage('invalid response (3.10)'+#13+
                 'command <'+strip(s)+'>'+#13+
                 'response <'+strip(r)+'>');
...
// send set serial format command "AT+Ups"
  s:='AT+U';
  ...
  if length(s)<>6 then s:='AT';                  // no action on error
  s:=s+EOL;
  e:=at2ok(s);
...
begin
  showmessage('(c)  Robert Rozee  2016'#13#13'Release 4 (28-may-2016)')
end;
